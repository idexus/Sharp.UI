...
public class MauiExtensionBuilder
{
    ...
    private readonly INamedTypeSymbol mainSymbol;
    private readonly INamedTypeSymbol wrappedType;
    ...
    public MauiExtensionBuilder(INamedTypeSymbol symbol, StringBuilder builder, bool isWrappedSymbol)
    {
        ...
        this.wrappedType = WrapBuilder.GetBaseType(symbol);
        if (wrappedType == null)
        {
            ...
            if (isWrappedSymbol)
                ...
                this.typeConformanceName = $"Sharp.UI.{WrapBuilder.GetInterfaceName(wrappedType)}";
        }
        else
        {
            this.typeConformanceName = $"Sharp.UI.{WrapBuilder.GetInterfaceName(wrappedType)}";
    ...
    public void Build()
    {
        if (noBaseType && isWrappedSymbol)
            builder.Append($@"
namespace {wrappedType.ContainingNamespace}
{{
    using Sharp.UI;

    public static class {WrapBuilder.GetTypeName(wrappedType)}GeneratedExtension
    {{");
        else
        builder.Append($@"
namespace Sharp.UI
{{
    ...
    void GenerateBindableProperties()
    {
        ...
        foreach (var inter in interfaces)
        {
            ...
            {
                var bindPropName = $"{wrappedType.ToDisplayString()}.{prop.Name}";
    ...
    {
        var wrapperAttribute = WrapBuilder.GetMauiWrapperAttributeData(mainSymbol);
        if (wrapperAttribute != null)
        {
            List<INamedTypeSymbol> interfaces = new List<INamedTypeSymbol>();

            var interfacesAttribute = WrapBuilder.GetAttachedInterfacesAttributeData(mainSymbol);
    ...
    {
        var info = new PropertyInfo(wrappedType, (IPropertySymbol)property);
        if (!notGenerateList.Contains(info.propertyName))
        {
    ...
    void GenerateExtensionMethod_List(PropertyInfo info, string typeName)
    {
        ...
        {{
            var mauiObject = MauiWrapper.GetObject<{wrappedType.ToDisplayString()}>(obj);
